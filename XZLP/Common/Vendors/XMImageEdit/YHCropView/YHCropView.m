...
- (void)setupImageView {
    ...
    if(cropRect.size.width>cropRect.size.height){
        [self setupWidthBigThanHeight];
        return;
    }else if(cropRect.size.width<cropRect.size.height){
        [self setupHeightBigThanWidth];
        return;
    }
...
- (void)setupWidthBigThanHeight
{
    CGRect cropRect = AVMakeRectWithAspectRatioInsideRect(self.image.size, self.insetRect);
    // 拉伸高到截图区域，满足正方形
    CGFloat imgHeight = cropRect.size.width;
    CGFloat newImageWidth = self.image.size.width*(imgHeight/self.image.size.height);
    // 拉伸后x需要缩小
    CGRect imageRect = CGRectMake(0, 0, newImageWidth, imgHeight);
    cropRect = CGRectMake(cropRect.origin.x, cropRect.origin.y-(cropRect.size.width-cropRect.size.height)/2, cropRect.size.width, cropRect.size.width);
    
    self.scrollView.frame = cropRect;
    self.scrollView.contentSize = imageRect.size;
    
    self.zoomingView = [[UIView alloc] initWithFrame:imageRect];
    self.zoomingView.backgroundColor = [UIColor clearColor];
    [self.scrollView addSubview:self.zoomingView];
...
//        self.imageView.backgroundColor = [UIColor greenColor];
    self.scrollView.contentOffset = CGPointMake((newImageWidth-imgHeight)/2, 0);
}
- (void)setupHeightBigThanWidth
{
    CGRect cropRect = AVMakeRectWithAspectRatioInsideRect(self.image.size, self.insetRect);
    CGRect imageRect = CGRectZero;
    // 拉伸宽到满足截图区域，满足正方形
    CGFloat imgWidth = cropRect.size.width;
    if(imgWidth==0){
        imgWidth = 1;
    }
    CGFloat newCropHeight = cropRect.size.width;
    CGFloat imgHeight = self.image.size.height*(imgWidth/self.image.size.width);
    imageRect = CGRectMake(0, 0, imgWidth, imgHeight);
    cropRect = CGRectMake(cropRect.origin.x,cropRect.origin.y+(cropRect.size.height-newCropHeight)/2, cropRect.size.width, newCropHeight);
...
//        self.scrollView.backgroundColor = [UIColor redColor];
//        self.imageView.backgroundColor = [UIColor greenColor];
    self.scrollView.contentOffset = CGPointMake(0, -(newCropHeight-imgHeight)/2);
}
#pragma mark -
