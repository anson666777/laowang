...
//    }
    ...
    if(self = [super initWithFrame:frame]) {
        ...
        self.endless = YES;
        self.aligment = UICollectionViewScrollPositionCenteredHorizontally;
        [self configureView];
    }
...
- (void)freshCarousel {
    ...
    if (self.endless)
        [self.carouselView scrollToItemAtIndexPath:[self originIndexPath] atScrollPosition:self.aligment animated:NO];
    else
    {
        if(self.flowLayout.style == CWCarouselStyle_Normal)
        {
            [self.carouselView scrollToItemAtIndexPath:self.currentIndexPath = [NSIndexPath indexPathForRow:0 inSection:0] atScrollPosition:self.aligment animated:NO];
        }
        else
        {
            [self.carouselView scrollToItemAtIndexPath:self.currentIndexPath = [NSIndexPath indexPathForRow:1 inSection:0] atScrollPosition:self.aligment animated:NO];
        }
    }
...
- (void)scrollViewWillBeginDecelerating:(UIScrollView *)scrollView {
    if(self.currentIndexPath != nil &&
       ...
       self.currentIndexPath.row >= 0) {
        ...
        [self.carouselView scrollToItemAtIndexPath:self.currentIndexPath atScrollPosition:self.aligment animated:YES];
    }
}
...
- (void)checkOutofBounds {
    ...
    if(self.currentIndexPath.row == [self infactNumbers] - 1) {
        ...
        [self.carouselView scrollToItemAtIndexPath:self.currentIndexPath atScrollPosition:self.aligment animated:NO];
        self.carouselView.userInteractionEnabled = YES;
...
- (void)pageControlClick:(UIPageControl *)sender {
    ...
    if(prePage - page == [self numbers] - 1) {
     ...
     [self.carouselView setContentOffset:CGPointMake(self.carouselView.contentOffset.x + CGRectGetWidth(self.carouselView.frame)*0.75, 0) animated:YES];
...
- (void)nextCell {
    ...
    if(self.currentIndexPath.row < [self infactNumbers] - maxIndex)
    {
        ...
        NSLog(@"contentOffsetX:%lf",self.carouselView.contentOffset.x);
        [self.carouselView setContentOffset:CGPointMake(self.carouselView.contentOffset.x + CGRectGetWidth(self.carouselView.frame)*0.75, 0) animated:YES];
//         [self.carouselView setContentOffset:CGPointMake(self.carouselView.contentOffset.x + CGRectGetWidth(self.carouselView.frame)*0.75, 0) animated:YES];
        self.currentIndexPath = indexPath;
    }
